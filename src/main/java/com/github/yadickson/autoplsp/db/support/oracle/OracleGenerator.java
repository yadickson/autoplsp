...
public class OracleGenerator extends Generator {
    ...
    public String getTablesQuery() {
        return "SELECT ut.table_name name,\n"
                ...
                + "     THEN 'Numeric'\n"
                + "     WHEN utc.data_type = 'DATE' OR utc.data_type LIKE '%TIMESTAMP%'\n"
                + "     THEN 'Date'\n"
                + "     ELSE 'Unknown'\n"
                + "     END fieldtype,\n"
                + "utc.column_id fieldposition,\n"
                + "DECODE(utc.nullable, 'Y', 0, 1) fieldminsize,\n"
                + "trunc(DECODE(utc.char_used, null, \n"
                + "(\n"
                + "CASE WHEN (utc.data_type = 'BLOB')\n"
                + "     THEN 2147483647 * 2\n"
                + "     WHEN (utc.data_type = 'CLOB' OR utc.data_type = 'NCLOB')\n"
                + "     THEN 2147483647\n"
                + "     ELSE TO_NUMBER(utc.data_precision)\n"
                + "     END\n"
                + "), utc.char_length)) fieldmaxsize,\n"
                + "trunc(NVL(utc.data_scale,0)) fieldscale,\n"
                + "DECODE(utc.nullable, 'N', 1, 0) fieldnotnull,\n"
                + "utc.data_default fielddefauldvalue,\n"
                + "CASE when utc.data_precision is not null\n"
                + "     then LPAD(9, utc.data_precision, '9')\n"
                + "     else null\n"
                + "     end fieldmaxnumbervalue,\n"
                + "CASE WHEN (utc.data_type = 'BLOB' OR utc.char_used = 'B')\n"
                + "     THEN 'Byte'\n"
                + "     WHEN (utc.data_type = 'CLOB' OR utc.data_type = 'NCLOB' OR utc.char_used = 'C')\n"
                + "     THEN 'Char'\n"
                + "     ELSE null\n"
                + "     END fieldcharused\n"
                + "FROM user_tables ut\n"
